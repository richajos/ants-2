...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        nearestPlace = None
        count = 0
        while checkPlace.name != "Hive":
            if not(len(checkPlace.bees)) == 0 and self.lower_bound <= count <= self.upper_bound:
                return random_bee(checkPlace.bees)
            else:
        ...
        if nearestPlace is None:
            ...
            return random_bee(nearestPlace.bees)
    ...
    def reduce_health(self, amount):
        ...
        for x in self.place.bees[:]:
            if self.health <= amount:
                x.reduce_health(amount+self.damage)
            else:
